...
def fetch_closed_positions_last_ndays(okx: "OKXClient", days: int = 7,
    ...
    - Dùng /account/positions-history?instType=SWAP&limit=100&after=...
    - Paginate nhiều lần bằng 'after' = posId của record cuối cùng.
    - Dừng khi:
        + cTime < cutoff (7 ngày trước)
        + hết dữ liệu (trả về < page_limit)
        + vượt max_pages (chặn vòng lặp vô tận).

    Trả về: list[dict] trades dạng:
      {
        "posId", "instId", "side", "sz",
        "openAvgPx", "closeAvgPx", "pnl", "cTime"
      }
    """
    ...
    logging.info(
        "[BT-FULL] Bắt đầu fetch positions-history %d ngày gần nhất (cutoff=%s UTC, %d ms)",
        days,
    ...
    while page < max_pages:
        ...
        if not raw:
            logging.info("[BT-FULL] Page %d: không còn dữ liệu, dừng.", page)
            break

        logging.info(
            "[BT-FULL] Page %d: nhận %d rows từ OKX (after=%s)",
            page,
            len(raw),
            after_cursor,
        )

        # OKX thường trả từ mới -> cũ, ta duyệt theo thứ tự này
        for d in raw:
            pos_id = d.get("posId")
            if not pos_id:
                continue
            ...
            try:
                ctime_ms = int(d.get("cTime", 0) or 0)
            except Exception:
                ctime_ms = 0

            # Nếu lệnh này quá cũ (< cutoff 7 ngày) -> dừng luôn (vì phía sau còn cũ hơn)
            if ctime_ms and ctime_ms < cutoff_ms:
                logging.info(
                    "[BT-FULL] Gặp lệnh cũ hơn %d ngày (posId=%s, cTime=%d) -> dừng fetch.",
                    days,
                    pos_id,
                    ctime_ms,
                )
            ...
            try:
                ...
                logging.error("[BT-FULL] Lỗi parse item %s: %s", d, e)

        # Nếu ít hơn page_limit => không còn page tiếp theo
        if len(raw) < page_limit:
            logging.info(
                "[BT-FULL] Page %d trả về < %d rows -> hết dữ liệu.",
                page,
                page_limit,
            )
            break

        # Chuẩn bị cursor cho page sau: dùng posId của record cuối cùng
        last = raw[-1]
        after_cursor = last.get("posId") or last.get("cTime")
        if not after_cursor:
            logging.info(
                "[BT-FULL] Không tìm được posId/cTime cho after_cursor -> dừng.",
            )
            break

    logging.info(
        "[BT-FULL] Hoàn tất fetch: tổng %d trades (distinct posId=%d).",
        len(all_trades),
        len(seen_pos_ids),
    )
    return all_trades
